"""
Σημείο εκκίνησης (entry point) του προγράμματος. Διαβάζει τις υπερπαραμέτρους από τη γραμμή
εντολών και από το config .yaml, φτιάχνει το αντικείμενο Performer και ξεκινά την εκπαίδευση
με αξιολόγηση σε κάθε epoch. Προτεραιότητα: command line > config > default.
...
def get_parser():
    ...
    # --- SCHEDULER ---
    parser.add_argument("--scheduler",      type=str,        default="ExponentialLR",              help="Type of scheduler: StepLR or ExponentialLR"                             )
    parser.add_argument("--step_size",      type=int,        default=20,                           help="Step size for StepLR"                                                   )
    parser.add_argument("--lr_gamma",       type=float,      default=0.9,                          help="Gamma for LR decay (0.1 for StepLR, 0.9 for ExpLR)"                     )
    # -------------------------------------
    ...
    parser.add_argument("--test_ten_crop",             type=str2bool,   default=True,                                                                                                       )
